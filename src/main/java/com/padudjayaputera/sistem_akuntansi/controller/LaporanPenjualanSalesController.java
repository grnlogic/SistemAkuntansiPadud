...
import com.padudjayaputera.sistem_akuntansi.service.LaporanPenjualanSalesService;

import jakarta.validation.Valid;
import lombok.RequiredArgsConstructor;

@RestController
@RequestMapping("/api/v1/laporan-penjualan")
@RequiredArgsConstructor
public class LaporanPenjualanSalesController {

    private final LaporanPenjualanSalesService laporanService;
    ...
    public ResponseEntity<LaporanPenjualanSales> createOrUpdateReport(@Valid @RequestBody LaporanPenjualanSalesRequest request) {
        return new ResponseEntity<>(laporanService.createOrUpdateReport(request), HttpStatus.CREATED);
    }

    @GetMapping
    @PreAuthorize(HAS_ACCESS_TO_PEMASARAN)
    public ResponseEntity<List<LaporanPenjualanSales>> getAllReports() {
